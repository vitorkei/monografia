...
as camadas \textbf{ocultas} são o processamento;
...
\textbf{Neurônios}, ou nós, são funções que recebem, como entrada, a saída de cada neurônio da camada anterior e devolvem um número, em geral entre 0 e 1 inclusos, cujo significado e como são usados variam de acordo com o trabalho em questão.
...
A forma mais comum de se utilizar neurônios em redes neurais artificiais é por meio da \textbf{combinação linear} dos neurônios da camada anterior, ou seja, multiplicação do valor dos neurônios da camada anterior pelos respectivos pesos, e pelo uso de uma \textbf{função de ativação não-linear}.
%Em redes neurais artificiais, um neurônio é ativado quando a combinação linear dos neurônios da camada anterior (soma do valor dos nós multiplicados pelos respectivos pesos) passa de um valor determinado pela função de ativação.
...
Seja $n$ o número de neurônios na camada anterior, $w_{i}$, $i = 1, ..., n$, os pesos das saídas de cada neurônio da camada anterior, e $a_{i}$, $i = 1, ..., n$, o valor de saída de cada neurônio da camada anterior
%e, por consequência, cada valor de entrada do neurônio atual,
...
Esse procedimento é feito em cada neurônio de cada camada da rede neural, cada um com seus respectivos pesos para a camada anterior.
...
O próximo passo é entender como os valores dos neurônios e os respectivos pesos são utilizados para devolver a resposta correta.
...
Naturalmente, quanto maior for o erro (valor calculado pela função de custo), mais incorreta foi a previsão, e isso permite avaliar o desempenho da rede neural.
...
Se o modelo tiver zero de erro em relação a esse conjunto, ele estará sofrendo de \textit{overfitting}: a inteligência se adequa tanto ao conjunto de treinamento que saberá o que fazer apenas nele.
Para determinar o grau de \textit{overfitting}, utiliza-se um \textbf{conjunto de testes}, exemplos independente do conjunto de treinamento, mas gerado pela mesma distribuição de probabilidade, escolhidos da mesma forma.
Um modelo que tenha bom desempenho, ou seja, erro baixo nos dois conjuntos não tem problemas de \textit{overfitting}.
%O mais desejável é minimizar o erro sobre todas as entradas possíveis ou, no mínimo, as esperadas em cenários reais, e o conjunto de treinamento dificilmente conseguirá abranger todos eles.
...
%após repetir esses passos diversas vezes, espera-se que a IA tenha aprendido o suficiente a resolver o problema em mãos.
...
Enquanto existe apenas uma camada de entrada e uma de saída, é possível haver uma ou mais camadas ocultas.
Se houver muitas camadas ocultas, a rede neural passa a ser chamada de rede neural profunda (\textit{deep neural network}).
Atualmente, não existe uma definição exata de quantas camadas a rede neural precisa ter para começar a ser classificada como profunda e, mesmo que houvesse, esse número provavelmente mudaria com o passar do tempo.
...
É possível haver mais de uma camada de convolução assim como pode haver mais de uma camada \textit{fully-connected} além da de saída, e isso pode ser mais vantajoso, uma vez que aumentar a profundidade da rede, em muitos casos, aparenta melhora a predição~\cite{Goodfellow-et-al-2016}.
...
Mesmo que não houvesse esse obstáculo computacional, é comum haver situações em que não é possível determinar qual ação retornará a maior recompensa. A figura \ref{fig:unknown_best_action} exemplifica um momento desse tipo.
...
%  \end{minipage}
%  \centering
  \caption{Quando a bola no \textit{Pong} ou os asteróides em \textit{Asteroids} ficam fora da tela, não é possível dizer qual a melhor ação a ser tomada por falta de informação. No \textit{Pong}, não é possível ver a bola somente quando algum dos jogadores marca um ponto, pois ela saiu da tela pelo lado esquerdo ou direito; quando isso ocorre, todas as ações tomadas têm o mesmo resultado, pois é preciso esperar alguns instantes para uma nova bola aparece e o jogo continuar.}
